    ...
    <main data-rail-flush className="relative flex min-h-0 flex-1 flex-col pt-20">
      <header className="relative z-[40] flex shrink-0 flex-wrap items-center gap-2.5 border-b border-edge-soft/40 bg-surface px-6 py-2.5">
        ...
        <div className="flex h-11 items-center gap-0.5 rounded-xl border border-edge-soft/55 bg-elevated p-1">
          <TabButton active={tab === "movies"} onClick={() => chooseTab("movies")} icon={<Film size={14} strokeWidth={2} />} label="Movies" count={library.movies.length} />
          <TabButton active={tab === "series"} onClick={() => chooseTab("series")} icon={<Tv size={14} strokeWidth={2} />} label="Shows" count={library.series.length} />
        </div>
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto px-6 pt-6 pb-16">
        {state.kind === "error" ? (
          ...
          <Notice text="Loading playlist..." />
        ) : selected ? (
          <SeriesDetail series={selected} onBack={() => setSelected(null)} onPlay={(ep) => playEpisode(selected, ep)} />
        ) : tab === "movies" ? (
          movies.length === 0 ? (
            <Notice text={emptyText(library.movies.length, "movies", q)} />
          ) : (
            <>
              <CapNote shown={Math.min(movies.length, RENDER_CAP)} total={movies.length} label="movies" />
          ...
          <Notice text={emptyText(library.series.length, "shows", q)} />
        ) : (
          <>
            <CapNote shown={Math.min(series.length, RENDER_CAP)} total={series.length} label="shows" />
            <div className="grid gap-x-4 gap-y-6" style={gridStyle}>
              {series.slice(0, RENDER_CAP).map((s) => (
                <VodCard
                  ...
                  logo={s.logo}
                  seed={s.title}
                  subtitle={`${s.episodes.length} ${s.episodes.length === 1 ? "episode" : "episodes"}`}
...
function emptyText(total: number, label: string, query: string): string {
  if (query) return `No ${label} match "${query}".`;
  if (total === 0) return `This playlist has no ${label}. It may be live channels only, or an Xtream login that exposes ${label} separately.`;
...
function CapNote({ shown, total, label }: { shown: number; total: number; label: string }) {
  if (total <= shown) return null;
  return (
    <p className="mb-4 text-[12.5px] text-ink-subtle">
      Showing {shown.toLocaleString()} of {total.toLocaleString()} {label}. Search to find the rest.
    </p>
  );
}
